...
class MapartController extends Component {
  ...
  handleGetPDNPaletteClicked = () => {
    ...
    const { coloursJSON, selectedBlocks, optionValue_modeNBTOrMapdat, optionValue_staircasing, optionValue_unobtainable } = this.state;
    let paletteText =
  ...
  presetToURL = () => {
    ...
    Object.keys(selectedBlocks).forEach((key) => {
      if (selectedBlocks[key] !== "-1" && coloursJSON[key].blocks[selectedBlocks[key]].presetIndex !== "CUSTOM") { // TODO enable custom blocks in presets
